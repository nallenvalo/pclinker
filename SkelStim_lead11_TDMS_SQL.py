...
with nidaqmx.Task() as writetask, nidaqmx.Task() as readtask:
    ...
    writetask.do_channels.add_do_chan('/Dev3/port0/line1')
    ...
    print(f"\nCurrently pulsing and collecting data, expected completion : {time.ctime(time.time() + t_sampling)}...\n")
...
def process_databases(database_input_df_dict): 
    ...
    for database_name, df in database_input_df_dict.items():
        ...
        table = database_name + '_table_' + f'{datetime.now().month}_{datetime.now().year}'
