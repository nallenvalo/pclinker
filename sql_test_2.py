...
pattern_ = [.51, .55, .57, -.55, -.5, -.58, .03, .02, .01, -.01]
fives = np.tile(pattern_, 1000)
        ...
        if end_index != NS : 
            ...
            if not peaks or not neg_peaks: 
                ...
                if new_indexes.size > 0:
                    end_idx = new_indexes[-1] + (8 * read_frequency) 
                    if end_idx < len(pulse_data) :
                        post_pulse_interval = pulse_data[new_indexes[-1] : end_idx]
                        unexpected_peaks += detect_peaks(post_pulse_interval, 1, True)
                        unexpected_peaks += detect_peaks(post_pulse_interval, 1, False)
                    if unexpected_peaks: 
                        print('entered')
                        normal_stim_ = False
...
def analyze_full_pulse_data(pulse_data, t, name, df_cycles : pd.DataFrame):
    ...
    if not pos_peaks or not neg_peaks: 
        ...
        df.at[idx, 'avg pos current'] = np.mean(pulse_data[pos_peaks] / resistance)
        ...
        df.at[idx, 'avg pos current'] = np.mean(pulse_data[neg_peaks] / resistance)
    ...
    df.at[idx, 'pulseDuration1'] = np.mean(df_cycles['stimulation period'].iloc[0])
...
for i, (key, value) in enumerate(pulse_channels_data_dict.items()):
    ...
    data_frame_cyles, normal_stim, indexes = process_data_by_interval(value * 10, samples_per_interval, num_samples, read_frequency, key)
    data_frame_pulse_summary = analyze_full_pulse_data(value * 10, time_array, key, data_frame_cyles)
    if normal_stim: 
        value = value[indexes]
...
def add_rows(engine, table_name, df : pd.DataFrame, chunk_size = 1000000):
...
def add_columns(engine, table_name, df : pd.DataFrame, chunk_size = 1000000):
...
def process_databases(database_input_df_dict): 
    ...
    for database_name, df in database_input_df_dict.items():
        ...
        if database_name == 'voltage': table = database_name + '_table_' + f'{datetime.now().month}_{datetime.now().year}'
        else : table = database_name + '_table'
