...
idx = 0
shorted_harvard_df = pd.DataFrame(columns = ['id',
                                             'Stimulator', 
                                             'voltage',
                                             'stimFreq', 
                                             'pulseOn'
...
for harvard in Stimulators_list: 
    ...
    print(f'harvard {harvard} plate rows : {plate_rows}')
    try : 
        most_recent_values = plate_rows.sort_values(by='day_time', ascending=False).head(1)
        ...
        shorted_harvard_df.at[idx, 'pulseOn'] = round(pulseOn, 5)
